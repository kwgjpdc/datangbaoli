<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="100px">
      ...
      <el-form-item label="借款人" prop="agreeDebtorName">
        <el-input v-model="queryParams.agreeDebtorName" placeholder="请输入借款人" clearable
  ...
  <!-- <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="loanList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="应收账款金额" align="center" prop="financingAmount" />
...
import { listEntry, delEntry } from "@/api/debt/entry";
...
/** 新增按钮操作 */
function handleAdd() {
  router.push({ name: 'Entry/detail/index', state: { pageNum: queryParams.value.pageNum } });
}
/** 新增按钮操作 */
function handleView(row) {
  const _entryId = row.id || ids.value
  router.push({ name: 'Entry/detail/index', state: { entryId: _entryId, viewFlag: true, pageNum: queryParams.value.pageNum } });
}

/** 修改按钮操作 */
function handleUpdate(row) {
  const _entryId = row.id || ids.value
  router.push({ name: 'Entry/detail/index', state: { entryId: _entryId, pageNum: queryParams.value.pageNum } });
}
