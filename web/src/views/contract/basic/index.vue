    ...
    <div class="app-container" v-loading="loading">
        ...
        <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="110px">
            ...
            <el-form-item label="保理申请人名称" prop="sellCustomerName">
                <CustomerSelect :option="customer.optiona" :showValue="customer.showValueSell"
                                :queryPropList="customer.queryPropList" :tablePropList="customer.tablePropList"
                                @selectRow="customerSelectSell" style="width:240px;"></CustomerSelect>
            </el-form-item>
            <!-- 买方客户名称 -->
        ...
        <el-table :data="contractList">
            ...
            <el-table-column label="合同编号" align="center">
                <template #default="scope">
                    <el-button link type="primary" @click="handleView(scope.row)">{{ scope.row.contractNo }}</el-button>
                </template>
            </el-table-column>
            <el-table-column label="合同名称" align="center" prop="otherContractName" />
            <el-table-column label="保理申请人名称" align="center" prop="factoringApplicantName" />
            <el-table-column label="核心企业名称" align="center" prop="coreEnterpriseName" />
            <el-table-column label="首次审批时间" align="center" prop="firstApproveDate" />
...
function handleAdd() {
    resetQuery();
    router.push({ name: 'Contract/detail', state: { pageNum: queryParams.value.pageNum } });
}
...
function handleUpdate(row) {
    resetQuery();
    router.push({ name: 'Contract/detail', state: { contractId: row.contractId, editFlag: true, pageNum: queryParams.value.pageNum } });
}

// 查看详细链接操作
function handleView(row) {
    resetQuery();
    router.push({ name: 'Contract/detail', state: { contractId: row.contractId, viewFlag: true, pageNum: queryParams.value.pageNum } });
}
