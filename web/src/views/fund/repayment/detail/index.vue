...
import { getFundPayInfo, addFundPayInfo, updateFundPayInfo } from "@/api/fund/info.js"
...
function getDebtInfoPage() {
    if (debtId.value) {
        loading.value = true;
        getFundPayInfo(debtId.value).then(response => {
...
function submitForm(statusFlag) {
    ...
    Promise.all([basicInfoRefForm, flowForm]).then(() => {
        ...
        if (!debtId.value) {
            //新增
            addFundPayInfo(debtInfoSave).then(response => {
            ...
            //修改
            updateFundPayInfo(debtInfoSave).then(response => {
...
function closePage() {
    const obj = { path: "/fund/info", query: { t: Date.now(), pageNum: routerQueryObj.value.pageNum } };
