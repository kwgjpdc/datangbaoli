...
<script setup>
import { getFundPoolInfo, addFundInfo, updateFundPoolInfo } from "@/api/fund/info.js"
...
/** 修改的时候查询客户基本信息列表 */
function getFundPoolInfoPage() {
    if (debtId.value) {
        loading.value = true;
        getFundPoolInfo(debtId.value).then(response => {
...
function submitForm(statusFlag) {
    ...
    Promise.all([basicInfoRefForm, flowForm]).then(() => {
        ...
        if (!debtId.value) {
            //新增
            addFundInfo(debtInfoSave).then(response => {
            ...
            //修改
            updateFundPoolInfo(debtInfoSave).then(response => {
...
function closePage() {
    const obj = { path: "/fund/fund/info/index", query: { t: Date.now(), pageNum: routerQueryObj.value.pageNum } };
