  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="90px">
      ...
      <el-form-item label="资金提供方" prop="provider">
        <el-select v-model="queryParams.params.provider" filterable placeholder="请选择资金提供方"
                   :style="{ width: '240px' }">
    ...
    <el-table v-loading="loading" :data="infoList" @selection-change="handleSelectionChange"  style="width: 100%"  @header-dragend="changeColwidth" border>
      <el-table-column label="资金编号" align="center" prop="drawNo" width="120" >
        <template #default="scope">
          <el-button link type="primary" @click="handleView(scope.row)">{{ scope.row.drawNo }}</el-button> </template>
        </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding" fixed="right"  width="120">
        <template #default="scope">
          ...
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-if="scope.row.status!=2" v-hasPermi="['draw:info:edit']">修改</el-button>
...
import { listFundDrawInfo, delFundDrawInfo, listFundPoolsum } from "@/api/fund/info";
...
function getList() {
  ...
  listFundDrawInfo(queryParams.value).then(response => {
...
function handleAdd() {
  ...
  router.push({ name: 'Fund/draw/detail/index', state: { pageNum: queryParams.value.pageNum } });
}
...
function handleView(row) {
  ...
  const _id = row.fundDrawId
  router.push({  name: 'Fund/draw/detail/index',  state: { debtId: _id, viewFlag: true, pageNum: queryParams.value.pageNum } });
}
/** 修改按钮操作 */
function handleUpdate(row) {
  const _id = row.fundDrawId
  console.log("@@@@@draw@@@draw@@@")
  console.log(row.fundDrawId)
  console.log(_id)
  router.push({ name: 'Fund/draw/detail/index', state: { debtId: _id, pageNum: queryParams.value.pageNum } });
}
...
function handleDelete(row) {
  const _ids = row.fundDrawId;
  proxy.$modal.confirm('是否确认删除资金提款编号为"' + _ids + '"的数据项？').then(function () {
    return delFundDrawInfo(_ids);
  }).then(() => {
    getList();
