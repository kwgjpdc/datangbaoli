...
import { getFundDrawInfo, addFundDrawInfo, updateFundDrawInfo } from "@/api/fund/info.js"
import basicInfo from './fundBasicInfo.vue'
import attachInfo from './fundAttachInfo.vue'
...
function getDebtInfoPage() {
    if (debtId.value) {
        loading.value = true;
        getFundDrawInfo(debtId.value).then(response => {
...
function submitForm(statusFlag) {
    ...
    Promise.all([basicInfoRefForm, flowForm]).then(() => {
        const formKeys = ['basicInfoRef', 'attachInfoRef', 'flowSearchRef'];
        ...
        if (!debtId.value) {
            //新增
            addFundDrawInfo(debtInfoSave).then(response => {
            ...
            //修改
            updateFundDrawInfo(debtInfoSave).then(response => {
...
function closePage() {
    const obj = { path: "/fund/fund/draw/info", query: { t: Date.now(), pageNum: routerQueryObj.value.pageNum } };
