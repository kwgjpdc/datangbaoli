<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="infoList" @selection-change="handleSelectionChange"  style="width: 100%" border>
      <el-table-column label="机构名称" align="center">
        <template #default="scope">
          <el-button link type="primary" @click="handleView(scope.row)">{{ scope.row.institutionName }}</el-button>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding" fixed="right"  width="260">
        <template #default="scope">
          ...
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['demo:info:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['demo:info:remove']">删除</el-button>
        </div>
        </template>
      </el-table-column>
    </el-table>
...
import { listInstitutionInfo, delInstitutionInfo } from "@/api/institution/info";
...
function handleAdd() {
  ...
  router.push({ name: 'Cust/institution/detail/index', state: { pageNum: queryParams.value.pageNum } });
}
...
function handleView(row) {
  ...
  router.push({  name: 'Cust/institution/detail/index',  state: { institutionId: _id, viewFlag: true, pageNum: queryParams.value.pageNum } });
}
/** 修改按钮操作 */
function handleUpdate(row) {
  const _id = row.institutionId
  router.push({ name: 'Cust/institution/detail/index', state: { institutionId: _id, pageNum: queryParams.value.pageNum } });
}
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除机构名称为"' + _names + '"的数据项？').then(function () {
