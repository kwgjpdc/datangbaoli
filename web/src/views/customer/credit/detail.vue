			...
			<div class="content-item-scroll">
				...
				>
					<el-collapse v-model="activeNames">
						<template v-if="routerQueryObj.isSanyi">
							<el-collapse-item title="三重一大额度" name="0">
						...
						<el-collapse-item title="授信信息" name="1">
							<el-row :gutter="15" style="margin-right: 30px">
									...
									<el-form-item label="担保方式" prop="guarantyWay">
										<el-radio-group
											:disabled="
												routerQueryObj.viewFlag || routerQueryObj.isSanyi
											"
											v-model="formData.guarantyWay"
										>
											<el-radio
												v-for="dict in credit_guaranty_way"
												:label="dict.value"
												:name="dict.value"
												:key="dict.value"
												>{{ dict.label }}</el-radio
									...
									<el-form-item label="额度种类组" prop="typeGroup">
										<el-select
											:disabled="
												routerQueryObj.viewFlag || routerQueryObj.isSanyi
											"
											v-model="formData.typeGroup"
											filterable
											placeholder="请选择"
										>
											<el-option
												v-for="item in cust_credit_type"
												:key="item.value"
												:label="item.label"
												:value="item.value"
											>
								...
								<el-col :xl="12" :lg="16" :sm="24">
									...
									>
										<el-radio-group
											v-model="formData.manageMode"
											:disabled="
												routerQueryObj.viewFlag || routerQueryObj.isSanyi
											"
										>
											<el-radio
												v-for="dict in lines_manage_mode"
												:label="dict.value"
												:name="dict.value"
												:key="dict.value"
											>
...
import { getInfo, addInfo, updateInfo } from "@/api/customer/credit";
...
const { cust_credit_type, lines_manage_mode, credit_guaranty_way } =
		...
		if (props.approveId) {
			...
			routerQueryObj.value.viewFlag =
				...
				newRoute.query.viewFlag === "true"
					? true
					: false;
			routerQueryObj.value.isSanyi =
				newRoute.query &&
				newRoute.query.isSanyi &&
				newRoute.query.isSanyi === "true"
					? true
					: false;
		}
	},
...
function submitForm(statusFlag) {
	...
	// 账户管理不需要在外层校验；
	Promise.all([validForm, flowForm])
		.then(() => {
