  ...
  <div class="app-container info-form-container" v-loading="loading" ref="appContainerView">
    <div class="content">
      ...
      <div class="content-item-scroll" ref="formCon">
        <basic-information id="basicInformation" :infoData="customerDetailInfo" ref="basicInformationRef"
          :customerId="customerId" :routerQueryObj="routerQueryObj"></basic-information>

        <credit-rating-info id="creditRatingInfo" :infoData="customerDetailInfo" :customerId="customerId"
          ref="creditRatingInfoRef" :routerQueryObj="routerQueryObj"></credit-rating-info>

        <companyInfo id="companyInfo" :infoData="customerDetailInfo.companyInfo" :customerId="customerId"
          :routerQueryObj="routerQueryObj" ref="companyInfoRef"></companyInfo>

        <client-file id="clientFile" ref="clientFileRef" :infoData="customerDetailInfo.commonFileList2"
          :customerId="customerId" :routerQueryObj="routerQueryObj"></client-file>

        <customer-info id="customerInfo" ref="customerInfoRef" :infoData="customerDetailInfo" :customerId="customerId"
          :routerQueryObj="routerQueryObj"></customer-info>

        <bank-info id="bankInfo" ref="bankInfoRef" v-if="infoMsg" :infoData="customerDetailInfo.bankInfoList" :customerId="customerId"
          :routerQueryObj="routerQueryObj"></bank-info>
...
import { addInfo, updateInfo, getInfo, getCustomerList } from "@/api/customer/index"
...
function submitForm(statusFlag) {
  ...
  // 账户管理不需要在外层校验；
  Promise.all([basicInformationForm, companyInfoForm, customerInfoForm]).then(() => {
    const formKeys = ['basicInformationRef', 'creditRatingInfoRef', 'companyInfoRef', 'customerInfoRef', 'clientFileRef', 'bankInfoRef'];
    ...
    if (!custCustomerlnfoSave.customerId) {
      //新增
      addInfo(custCustomerlnfoSave).then(response => {
      ...
      //修改
      updateInfo(custCustomerlnfoSave).then(response => {
...
function onScroll() {
  // 获取所有锚点元素
  const navContents = document.querySelectorAll('.content div.content-item-scroll')
  // 所有锚点元素的 offsetTop
  const offsetTopArr = []
...
// 关闭详情标签页
function closePage() {
  const obj = { path: "/customer/customer/basic", query: { t: Date.now(), pageNum: route.query.pageNum } };
