...
<!--      </el-form-item>-->
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" fixed="right" width="200">
        <template #default="{ row }">
          <div class="button-display">
            <el-button link type="primary" v-if="row.status === '4'" icon="Edit" @click="selectMeeting(row)">人员选择</el-button>
            <el-button link type="primary" icon="Edit" @click="diligenceReview(row)"
                       v-if="row.status === '5' && showButtom(row)">尽调评审</el-button>
          </div>
        </template>
      </el-table-column>
    ...
    <SelectMeetingsComponents
        :visible="visibleMeetings"
        @confirm="personnelMeeting"
        @cancel="visibleMeetings = false"
    ></SelectMeetingsComponents>
    <DiligenceReviewComponents
        :visible="visibleReview"
        :userInfo="Adjustment"
        @confirm="reviewCallback"
        @cancel="visibleReview = false"
    ></DiligenceReviewComponents>
  </div>
</template>

<script setup name="Info">
import SelectMeetingsComponents from "./components/selectMeetingsForm.vue";
import DiligenceReviewComponents from "./components/diligenceReviewForm.vue";
import {
  listDiligence,
  getDiligence,
  delDiligence,
...
const projectDiligenceStatusList = ref(['4', '5']);
...
const visibleMeetings = ref(false)
...
function selectMeeting(row) {
  visibleMeetings.value = true
  Adjustment.value = row
}
async function personnelMeeting(values) {
  const copyValues = {
    ...deepClone(Adjustment.value),
    ...values
  }
  let resData = await updateDiligence(copyValues)
  if(resData.code === 200) {
    visibleMeetings.value = false
    getList();
    ElMessage({
      message: '提交成功.',
      type: 'success',
...
function diligenceReview(row) {
  Adjustment.value = row
  visibleReview.value = true
}

async function reviewCallback(values) {
  const cpoyValues = deepClone(values)
  let resData = await diligenceCommitOpinion(cpoyValues)
  if(resData.code === 200) {
...
function showButtom(row) {
  if(!row.projReviewCommitteeDecisionList) return;
  const user = row.projReviewCommitteeDecisionList.find((item) => item.userId === userStore.$state.id)
  console.log(user)
  if (user && user.decision) {
    if(user.reviewPeopleType === '1') return true;
    return  false
  }
  return true
}
