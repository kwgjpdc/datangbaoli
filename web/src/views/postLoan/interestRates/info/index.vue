<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="loanList">
      ...
      <el-table-column label="放款方式" align="center" prop="loanWay">
        <template #default="scope">
          <dict-tag :options="cont_pay" :value="scope.row.loanCurrency" />
        </template>
      </el-table-column>
        ...
        <template #default="scope">
          <div class="button-display">
            <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
              v-hasPermi="['postLoad:financing:edit']">修改</el-button>
...
<script setup name="postLoan-interestRates">
import QueryParams from "@/components/QueryParams"
import { listInfo } from "@/api/postLoan/interestRates"
import { formatMoney } from "@/utils/formatMoney"
const { proxy } = getCurrentInstance();
const route = useRoute();
const router = useRouter();
const { sys_currency_type, lend_buss_product, cont_pay } = proxy.useDict('sys_currency_type', 'lend_buss_product', 'cont_pay');//下拉框字典
...
function handleView(row) {
  ...
  router.push({ path: '/postLoan/interestRates/detail', query: { loanId: _id, viewFlag: true, pageNum: queryParams.value.pageNum } });
}

/** 录入按钮操作 */
function handleUpdate(row) {
  const _id = row.loanId
  router.push({ path: '/postLoan/interestRates/detail', query: { loanId: _id, pageNum: queryParams.value.pageNum } });
}
