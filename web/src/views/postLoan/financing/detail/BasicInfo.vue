        ...
        <el-row :gutter="15" style="margin-right: 30px;">
            ...
            <el-form-item label="入账冲销项目" prop="po.entryItem">
              <el-checkbox-group v-model="formData.po.entryItem">
                <template v-for="dict in payment_reverse_item" :key="dict.value">
                  <el-checkbox v-if="dict.remark && dict.remark.indexOf('融资回款冲销') > -1" :label="dict.value" :name="dict.value">{{ dict.label }}</el-checkbox>
                </template>
              </el-checkbox-group>
            </el-form-item>
          </el-col>
          ...
          <el-col :xl="6" :lg="8" :sm="12" :xs="24">
            <el-form-item label="扣账金额" prop="po.deductionAmount" :rules="rules['po.deductionAmount']">
              <ElPriceInput :form="formData.po" prop="deductionAmount" :rules="rules['po.deductionAmount']"
          ...
          <el-col :xl="6" :lg="8" :sm="12" :xs="24">
            <el-form-item label="还款来源" prop="po.repaymentSource">
              <el-select v-model="formData.po.repaymentSource" filterable placeholder="请选择">
                <el-option v-for="item in source_of_repayment" :key="item.value" :label="item.label"
                  :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
...
// 入账冲销项目选项筛选
// watch(payment_reverse_item.value, (newValue) => {
//   console.log('入账冲销项目选项筛选',newValue)
//   if (newValue && newValue.length !== 0) {
//     let newArray = newValue.filter(item => item.remark.indexOf("融资回款冲销") < 0)
//     payment_reverse_item.value = newArray
//   }
// }, { immediate: true, deep: true })
