    ...
    <el-row :gutter="15" class="form-row">
      <el-col :xl="6" :lg="8" :sm="12" :xs="24">
        <el-form-item label="入账账号"
                      prop="entryAccount">
          <el-select v-model="formData.entryAccount" placeholder="请选择入账账号">
            <el-option v-for="item in sys_entry_account" :key="item.value" :value="item.value">{{ item.label }} - {{ item.value }}</el-option>
          </el-select>
        </el-form-item>
      </el-col>
      <el-col :xl="6" :lg="8" :sm="12" :xs="24">
        <el-form-item label="对价日期"
                      prop="considerationDate">
          <el-date-picker type="date" v-model="formData.considerationDate" value-format="YYYY-MM-DD" placeholder="请选择对价日期"></el-date-picker>
        </el-form-item>
      </el-col>
    </el-row>
  </el-form>
...
const { soldout_payment_method,
  soldout_capital_type, sys_entry_account } = proxy.useDict("soldout_payment_method",
    "soldout_capital_type", "sys_entry_account");//下拉框字典
