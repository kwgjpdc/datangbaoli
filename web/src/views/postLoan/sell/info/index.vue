<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="loanList">
      ...
      <el-table-column label="放款编号" align="center" prop="loanNo" fixed min-width="240" />
      <el-table-column label="管理同意书编号" align="center" prop="agreementNo" fixed min-width="240" />
      <el-table-column label="放款余额" align="center" prop="loanBalance" min-width="150">
        <template #default="scope">
          <span>{{ formatMoney(scope.row.loanBalance) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="放款到期日" align="center" prop="loanExpireDate" min-width="120">
        <template #default="scope">
...
import { listData } from "@/api/postLoan/sell"
...
function handleSell() {
  ...
  router.push({ path: '/postLoan/soldOut/sell/detail', query: { loanId: _id, viewFlag: true, pageNum: queryParams.value.pageNum } });
}
...
function handleUpdate(row) {
  const _id = row.loanId
  router.push({ path: '/postLoan/soldOut/sell/detail', query: { loanId: _id, pageNum: queryParams.value.pageNum } });
}
