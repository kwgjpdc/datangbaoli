  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="expressionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:expression:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:expression:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="expressionRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入表达式名称" maxlength="32" />
        </el-form-item>
        <el-form-item label="内容" prop="expression">
          <el-input v-model="form.expression" placeholder="请输入表达式内容" maxlength="32" />
        </el-form-item>
...
function handleUpdate(row) {
  ...
  getExpression(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["expressionRef"].validate(valid => {
    if (valid) {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除流程达式编号为"' + _ids + '"的数据项？').then(function() {
    return delExpression(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('flowable/expression/export', {
    ...queryParams.value
