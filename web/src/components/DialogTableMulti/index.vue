<template>
	<!-- 添加或修改用户配置对话框 -->
	<el-dialog
		:title="multiQueryChild.title"
		:model-value="multiQueryChild.open"
		width="700px"
	...
	>
			...
			<el-row justify="center">
				<el-col :span="24">
					<el-table
						...
						v-show="tableDataShow.length"
					>
						<el-table-column
							:prop="item.value"
							:label="item.label"
							v-for="item in propShow"
							:key="item.value"
							align="center"
						>
		...
		<div class="page">
			<el-pagination
				...
				:page-size="multiQueryChild.pageSize"
				:current-page="multiQueryChild.currentPage"
				layout="total, prev, pager, next"
				:total="multiQueryChild.total"
			>
			</el-pagination>
		</div>
	</el-dialog>
			...
			let findList = propShow.value.filter(element => {
				if (!formInput.value[element.value]) return true;
					...
					return (
						...
						item[element.value]
							.toString()
							.indexOf(formInput.value[element.value].toString()) > -1
					);
				}
			});
